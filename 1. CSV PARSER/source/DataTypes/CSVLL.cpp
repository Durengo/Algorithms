...
CSV::CSV *CSVLL::getHead(){
  return head;
}

CSV::CSV *CSVLL::getPTR(int index){
  if(index >= length() || index < 0){
	std::cout << "\nINDEX OUT OF BOUNDS\n";
	return nullptr;
  }
...
void CSVLL::add(const std::string &category) {
  auto *node = new CSV::CSV(category);
  if (head == nullptr) {
	head = node;
	std::cout << "New Category Created!" << "\"" << category << "\"\n";
...
void CSVLL::remove() {
  ...
  if (head->next == nullptr) {
	...
	head->next->freeMem();
	head = nullptr;
	std::cout << "CATEGORY " << "\"" + tempstr + "\"" << " REMOVED\n";
	std::cout << "LL LOCATION: BACK\n";
	return;
  }
...
void CSVLL::remove(int index) {
  ...
  while (temp != nullptr) {
	if (count == index - 1) {
	  ...
	  std::cout << "Category Removed: " << "\"" << tempstr << "\"\n";
	  std::cout << "LL LOCATION: AT INDEX: " << index << "\n";
	  break;
	}
	count++;
