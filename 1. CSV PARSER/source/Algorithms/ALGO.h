...
class CSVQUEUE{
 private:
  ...
  int count;
  DTYPES::CSVLL *LOCALLIST;
  //std::vector<int> STORAGE;

  //BACK>---------FRONT>

 public:
  CSVQUEUE(DTYPES::CSVLL *LIST);
  ~CSVQUEUE();
  int size();
  int *front();
  int *back();
...
void LSearch(std::string argument, DTYPES::CSVLL *LIST);
void LSearch(std::string argument, ALGO::AXIS axis, int index, DTYPES::CSVLL *LIST);

void BSearch(std::string argument, DTYPES::CSVLL *LIST);
...
void ISort(bool lowtohigh, DTYPES::CSVLL *LIST, int index);
...
void REVERSE(bool colorrow, DTYPES::CSVLL *LIST, int index);

void RRange(int key, int count, DTYPES::CSVLL *LIST);
