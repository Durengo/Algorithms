...
void database::insert(item &item) {
  ...
  if (sqlite3_bind_text(stmt, 2, item.getName().c_str(), item.getName().length(), SQLITE_STATIC) != SQLITE_OK) {
	printf("\nCould not bind text.\n");
	return;
  }
  ...
  if (sqlite3_bind_text(stmt, 4, item.getAddress().c_str(), item.getAddress().length(), SQLITE_STATIC) != SQLITE_OK) {
	printf("\nCould not bind text.\n");
	return;
  }
  if (sqlite3_bind_int(stmt, 5, item.getSalary()) != SQLITE_OK) {
	printf("\nCould not bind int.\n");
	return;
  }
